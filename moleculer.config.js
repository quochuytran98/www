...
let streamToElastic = null;
try {
	console.log('streamToElastic start !!', 'index: ', process.env.ES_INDEX, process.env.ES_HOST, 'username:', process.env.ES_USER, 'password: ', process.env.ES_PASSWORD);
	...
	streamToElastic.on('insert', (stats) => {
		console.log('[Elasticsearch] => [Insert]:', stats);
	});

	streamToElastic.on('unknown', (line, error) => {
		console.log('[Elasticsearch] => [Unknown] => [Line]:', line);
		console.log('[Elasticsearch] => [Unknown] => [Error]:', error);
	});
} catch (error) {
	console.log('[Elasticsearch] => [Exception] :>> ', error);
}
